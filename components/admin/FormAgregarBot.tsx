...
interface FormAgregarBotProps {
  addBotAction: (data: PlayerFormData) => Promise<{ success: boolean; message?: string; error?: string }>;
}

const FormAgregarBot = ({ addBotAction }: FormAgregarBotProps) => {
  ...
  const onSubmit = async (data: PlayerFormData) => {
    ...
    try {
      const result = await addBotAction(data);

      if (result.success) {
        toast.success("¡Bot agregado!", { description: result.message });
        form.reset();
        setTimeout(() => {
          router.push("/admin");
        }, 1500);
      } else {
        toast.error("Error al agregar bot", { description: result.error });
      }
    } catch (error) {
    ...
    <div className="container mx-auto p-6 max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-center">
          Completa los datos del bot para agregarlo al sistema
        </p>
      </div>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  Nivel de habilidad del bot (80-99)
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
